      ...
      <div className="bg-white rounded-3xl max-w-lg w-full overflow-hidden shadow-2xl border border-slate-100 animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="p-6 bg-slate-900 text-white flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="text-lg font-bold">Transaction Inspection</h3>
        ...
        <div className="p-6 space-y-6 max-h-[80vh] overflow-y-auto">
          ...
          <div className="flex items-center justify-between pb-5 border-b border-slate-100">
            <div>
              ...
              <div className="flex items-center space-x-2 mt-1 text-xs text-slate-500">
                <span className="font-semibold text-slate-800">{transaction.merchantName}</span>
                <span>•</span>
          ...
          <div className="space-y-3">
            ...
            {breakdowns.length > 0 ? (
              <div className="bg-slate-50 rounded-2xl p-4 border border-slate-200/80 space-y-3">
                {breakdowns.map((b, idx) => (
                  <div key={idx} className="flex items-start justify-between text-xs pb-2 border-b border-slate-200/60 last:border-0 last:pb-0">
                    <div>
                      <div className="font-bold text-slate-800">{b.ruleName}</div>
            ...
            <div className="bg-gradient-to-r from-emerald-50 to-teal-50 rounded-2xl p-4 border border-emerald-100 flex items-center justify-between">
              <div>
                <span className="text-xs font-bold text-emerald-900">Total Net Reward</span>
              ...
              <div className="text-right">
                {cashbackReward && (
                  <div className={`text-base font-black ${isRefunded ? 'text-rose-600 line-through' : 'text-emerald-700'}`}>
                    ₹{Number(cashbackReward.amount).toFixed(2)} Cashback
                  </div>
                )}
                {pointsReward && (
                  <div className={`text-xs font-bold ${isRefunded ? 'text-rose-600 line-through' : 'text-amber-700'}`}>
          ...
          <div className="pt-2">
            {!isRefunded ? (
              <div className="space-y-3">
                <div className="bg-amber-50 p-3 rounded-xl border border-amber-200 text-xs text-amber-800 flex items-start space-x-2">
                ...
                <button
                  ...
                  className="w-full flex items-center justify-center space-x-2 py-3 px-4 bg-rose-600 hover:bg-rose-700 text-white font-bold rounded-xl shadow-md transition active:scale-98 disabled:opacity-50"
                >
              ...
              <div className="bg-rose-50 p-4 rounded-2xl border border-rose-200 text-center text-xs text-rose-800 space-y-1">
