...
export const OffersPage: React.FC<OffersPageProps> = ({ campaigns, onOpenCreateTxn }) => {
 ...
 className="flex items-center space-x-2 px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-semibold rounded-xl transition"
 >
 ...
 <span className="text-[11px] font-semibold uppercase tracking-wider px-2.5 py-1 rounded-full bg-emerald-50 text-emerald-700">
 {c.category}
 </span>
 ...
 <div className="mt-4">
 <h3 className="text-lg font-bold text-slate-900 leading-snug">{c.name}</h3>
 ...
 <span className="font-bold text-emerald-600">+{c.bonusPercentage}% Extra</span>
 </div>
 {c.bonusPoints && c.bonusPoints > 0 ? (
 <div className="flex justify-between">
 <span>Bonus Points:</span>
 <span className="font-bold text-amber-600">+{c.bonusPoints} PTS</span>
 </div>
 ) : null}
 {c.minTransactionAmount && (
 <div className="flex justify-between">
 <span>Min Spend:</span>
 <span className="font-medium text-slate-800">₹{c.minTransactionAmount}</span>
 </div>
 )}
 {c.maxReward && (
 <div className="flex justify-between">
 <span>Cap Limit:</span>
 <span className="font-medium text-slate-800">₹{c.maxReward}</span>
 </div>
 )}
 </div>
 </div>
