    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-br from-emerald-600 to-teal-800 rounded-3xl p-8 text-white shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-6 relative overflow-hidden">
        <div>
          ...
          <div className="text-3xl sm:text-4xl font-black mt-2 tracking-widest bg-white/10 px-4 py-2 rounded-2xl border border-white/20 inline-block">
        ...
        <button
          onClick={handleCopy}
          className="flex items-center justify-center space-x-2 px-6 py-3 bg-white text-emerald-800 hover:bg-emerald-50 font-bold rounded-2xl shadow-lg transition active:scale-95"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm md:col-span-1">
          ...
          <form onSubmit={handleInvite} className="space-y-3">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-2.5 bg-slate-900 hover:bg-slate-800 text-white text-xs font-bold rounded-xl transition active:scale-95 disabled:opacity-50 mt-2"
            >
        ...
        {/* Referrals Table */}
        <div className="bg-white rounded-3xl border border-slate-200 shadow-sm md:col-span-2 overflow-hidden flex flex-col justify-between">
          <div>
            <div className="p-6 border-b border-slate-100">
              <h2 className="text-base font-bold text-slate-900">Your Referrals &amp; Payouts</h2>
                ...
                referrals.map((ref) => (
                  <div key={ref.id} className="p-4 flex items-center justify-between hover:bg-slate-50/50">
                    <div className="flex items-center space-x-3">
                      <div className="h-9 w-9 rounded-xl bg-slate-100 flex items-center justify-center font-bold text-slate-700 text-sm">
                        {ref.referredUserName.charAt(0)}
                      </div>
                      <div>
                        <div className="text-sm font-bold text-slate-900">{ref.referredUserName}</div>
                    ...
                    <div className="flex items-center space-x-4">
                      {ref.status === 'COMPLETED' ? (
                        ...
                        <div className="flex items-center space-x-2">
                          <span className="px-2.5 py-0.5 rounded-full bg-amber-100 text-amber-800 text-xs font-bold flex items-center space-x-1">
