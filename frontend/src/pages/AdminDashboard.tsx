  ...
  return (
    <div className="space-y-6">
        ...
        <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="p-6 border-b border-slate-100 flex items-center justify-between">
            ...
            <button
              ...
              className="flex items-center space-x-1.5 px-3.5 py-2 bg-slate-900 hover:bg-slate-800 text-white rounded-xl text-xs font-bold shadow-sm"
            >
            ...
            <table className="w-full text-left text-xs">
              <thead className="bg-slate-50 text-slate-500 uppercase tracking-wider border-b border-slate-200/80">
                <tr>
                  ...
                  <th className="py-3.5 px-4 font-bold">Merchant Partner</th>
                  <th className="py-3.5 px-4 font-bold">Reward Value</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {rules.map((r) => (
                  <tr key={r.id} className="hover:bg-slate-50/60">
                    <td className="py-3.5 px-4 font-bold text-slate-900">{r.category}</td>
                    ...
                    <td className="py-3.5 px-4 text-right space-x-2">
                      <button
                        ...
                        className={`p-1.5 rounded-lg border transition ${
                          r.active ? 'text-amber-600 border-amber-200 hover:bg-amber-50' : 'text-emerald-600 border-emerald-200 hover:bg-emerald-50'
                        }`}
                      ...
                      <button
                        ...
                        className="p-1.5 rounded-lg text-rose-600 border border-rose-200 hover:bg-rose-50 transition"
                        title="Delete Rule"
                      >
            ...
            <table className="w-full text-left text-xs">
              <thead className="bg-slate-50 text-slate-500 uppercase tracking-wider border-b border-slate-200/80">
                <tr>
                  ...
                  <th className="py-3.5 px-4 font-bold">Category / Target</th>
                  <th className="py-3.5 px-4 font-bold">Booster Benefit</th>
                  ...
                  <tr key={c.id} className="hover:bg-slate-50/60">
                    ...
                    <td className="py-3.5 px-4 text-right space-x-2">
                      <button
                        ...
                        className="p-1.5 rounded-lg text-amber-600 border border-amber-200 hover:bg-amber-50 transition"
                      >
                        <Power className="h-4 w-4" />
                      </button>
                      <button
            ...
            <table className="w-full text-left text-xs">
              <thead className="bg-slate-50 text-slate-500 uppercase tracking-wider border-b border-slate-200/80">
                <tr>
                  ...
                  <th className="py-3.5 px-4 font-bold">Merchant / Category</th>
                ...
                {transactions.map((t) => {
                  ...
                  return (
                    <tr key={t.id} className="hover:bg-slate-50/60">
                      <td className="py-3.5 px-4 font-bold text-slate-900">TXN-{t.id}</td>
                      <td className="py-3.5 px-4">
                        <div className="font-bold text-slate-900">{userObj?.name || `User #${t.userId}`}</div>
                      ...
                      <td className="py-3.5 px-4 text-right">
                        {!isRefunded ? (
                          <button
                            ...
                            className="px-3 py-1 bg-rose-50 hover:bg-rose-100 text-rose-700 font-bold rounded-lg border border-rose-200 text-xs transition"
                          >
                            Refund &amp; Reverse
                          </button>
                        ) : (
                          <span className="text-[11px] font-semibold text-slate-400">Reversed</span>
                        )}
                      </td>
                    </tr>
                  );
            ...
            <table className="w-full text-left text-xs">
              <thead className="bg-slate-50 text-slate-500 uppercase tracking-wider border-b border-slate-200/80">
                <tr>
                  ...
                  <th className="py-3.5 px-4 font-bold">Referral Code</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {users.map((u) => (
                  <tr key={u.id} className="hover:bg-slate-50/60">
                    ...
                    <td className="py-3.5 px-4 font-mono font-bold text-slate-700">{u.referralCode}</td>
      ...
      {showRuleModal && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl space-y-4">
            <h3 className="text-lg font-bold text-slate-900">Create Reward Rule</h3>
            <div className="space-y-3 text-xs">
              ...
              <div>
                <label className="font-bold text-slate-700 block mb-1">Merchant Partner</label>
                <input
                  type="text"
                  className="w-full p-2.5 border rounded-xl"
      ...
      {showCampaignModal && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl space-y-4">
            <h3 className="text-lg font-bold text-slate-900">Launch Campaign Booster</h3>
            <div className="space-y-3 text-xs">
              <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="font-bold text-slate-700 block mb-1">Bonus Cashback (%)</label>
                  <input
                    type="number"
                    step="0.5"
                    className="w-full p-2.5 border rounded-xl font-bold"
            ...
            <div className="flex justify-end space-x-2 pt-3">
              ...
              <button
                ...
                className="px-4 py-2 text-xs font-bold bg-amber-600 text-white rounded-xl shadow"
              >
                Launch Campaign
              </button>
            </div>
